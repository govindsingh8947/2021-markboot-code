...
class TournamentDetailsPage extends StatefulWidget {
  ...
  String type;
  String subType; //@required required required
  TournamentDetailsPage(
      {required this.snapshot, required this.type, required this.subType});
  @override
...
class _TournamentDetailsPageState extends State<TournamentDetailsPage> {
  ...
  late List<String> pendingPostList;
  ...
  Future<void> init() async {
    try {
      ...
      // user details......
      userName = prefs.getString("userName")!; //! is added till next 4 lines
      userPhone = prefs.getString("userPhoneNo")!;
      userEmailId = prefs.getString("userEmailId")!;
      tournamentList =
          prefs.getStringList("internshiplist") ?? []; //new List();
      ...
      if (tournamentList.contains(widget.snapshot.id) ||
          tournamentMapDb.containsKey(widget.snapshot.id)) {
        isApplied = true;
        setState(() {});
      }

      localQuestionMapList =
          (widget.snapshot.data() as dynamic)["questionList"] ?? []; //new List();
      ...
      debugPrint("TOURN DETAILS ${(widget.snapshot.data() as dynamic)["questionList"]}");
    } catch (e) {
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                          ...
                          image: DecorationImage(
                              fit: BoxFit.cover,
                              image: NetworkImage((widget.snapshot.data() as dynamic)["imgUri"]))),
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Container(
                              ...
                              child: Text((widget.snapshot.data() as dynamic)["taskTitle"] ?? "",
                                  style: TextStyle(
                                      fontSize: 20,
                            ...
                            Row(
                              children: <Widget>[
                                CircleAvatar(
                                  radius: 25,
                                  backgroundImage:
                                      NetworkImage((widget.snapshot.data() as dynamic)["logoUri"]),
                                ),
                                SizedBox(
                                  width: 10,
                                ),
                                Text((widget.snapshot.data() as dynamic)["companyName"] ?? "",
                                    style: TextStyle(
                                        fontSize: 15,
                                        fontWeight: FontWeight.w500,
                                        color: Colors.white))
                      ...
                      Container(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              Text(
                                (widget.snapshot.data() as dynamic)["companyName"] ?? "",
                                style: TextStyle(
                                    fontSize: 15,
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: <Widget>[
                                ...
                                Text(
                                  (widget.snapshot.data() as dynamic)["reward"] ?? "0",
                                  style: TextStyle(
  ...
  tournamentService() async {
    try {
      ...
      List submittedBy = (widget.snapshot.data() as dynamic)["submittedBy"] ?? []; //new List();
      debugPrint("ALREADY ${(widget.snapshot.data() as dynamic)["submittedBy"]}");
      ...
      tournamentMapDb[widget.snapshot.id] = false;
      pendigAmount = (int.parse(pendigAmount) + //?? "0" removed after amount
              int.parse((widget.snapshot.data() as dynamic)["reward"] ?? "0"))
          .toString();
      debugPrint("PendingAMOUNT $pendigAmount");
      debugPrint("RReward ${(widget.snapshot.data() as dynamic)["reward"]}");
      ...
      await FirebaseFirestore.instance
          ...
          .set({"submittedBy": submittedBy}, SetOptions(merge: true));
      tournamentList.add(widget.snapshot.id);
