...
class _GeneralTabState extends State<GeneralTab>
  ...
  getpref() async {
    ...
    String userId =
        prefs.get("userId") as String; //String userId = prefs.get("userId");
    String userPhone = prefs.get("userPhoneNo") as String;
    print(userId);
    List mapi_a = [];
    List mapi_r = [];
    ...
    for (DocumentSnapshot snapshot in snaps) {
      for (var map in (snapshot.data() as dynamic)["submittedBy"]) {
        //data["submittedBy"]) {
    ...
    //print(snaps[0].data);
    for (DocumentSnapshot snapshot in snaps) {
      if ((snapshot.data() as dynamic)["user phone"] == userPhone) {
        //if (snapshot.data["user phone"] == userPhone) {
        if ((snapshot.data() as dynamic)["status"] == "approved") {
          //if (snapshot.data["status"] == "approved") {
          Map<String, dynamic> a = {
            "status": "approved",
            "reward": (snapshot.data() as dynamic)["reward"], //data["reward"],
            "offerName": (snapshot.data() as dynamic)["offer name"], //data["offer name"],
            "logoUri": (snapshot.data() as dynamic)["logoUri"]//data["logoUri"],
          };
          mapi_a.add(a);
        } else if ((snapshot.data() as dynamic)["status"] == "rejected") {
...
//      print(maps_offers_rejected.length);
    ...
    for (DocumentSnapshot snapshot in snaps) {
      for (var map in (snapshot.data() as dynamic)["appliedBy"]) {
        //data["appliedBy"]) {
