...
class OffersPageDetails extends StatefulWidget {
  ...
  String type;
  String subType; //@required ,add required ,required
  OffersPageDetails(
...
class _OffersPageDetailsState extends State<OffersPageDetails> {
  ...
  Future<void> init() async {
    try {
      ...
      if (pendingPostList.contains(widget.snapshot.id.toString())) {
        isApplied = true;
        setState(() {});
      }
      ...
      userData = snapshot.data as Map<String, dynamic>; //as is added
  ...
  Future<void> applyPostService() async {
    try {
      ...
      pendingPost[widget.snapshot.id] = false;
