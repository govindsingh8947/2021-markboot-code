...
class _CashbacksPageDetailsState extends State<CashbacksPageDetails>
  ...
  late File _workedImgFile = File("");
  late List<String> pendingPostList;

  //this is added later
  late DocumentSnapshot<Object?> localSnapshot;
  ...
  late AnimationController submitAnimCont;
  late Animation animation;
  late Animation submitAnim;
  late String name, emailId, userId;
  ...
  Future<void> init() async {
    try {
      ...
      name = prefs.getString("userName")!; //! is added to next 4 lines
      ...
      for (DocumentSnapshot doc in tasks) {
        ...
        if (widget.snapshot.id == doc.id) {
          List users = doc.get("submittedBy"); //doc.data["submittedBy"];
          print("GOt it");
          print(users);
  ...
  Future<void> applyPostService() async {
    try {
      ...
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          ...
          .get();
      //DocumentSnapshot<Object?> localSnapshot;//<object?> is added
      ...
      if (workedImgFileUri != null) {
        ...
        await _firestore
            ...
            .set({"offersSubmittedBy": offersList}, SetOptions(merge: true));

        offersListDB[widget.snapshot.id] = false;

        await _firestore
            .collection("Users")
            .doc(phoneNo)
            .set({"offersList": offersListDB}, SetOptions(merge: true))
            .whenComplete(() {})
      ...
      offersListPrefs.add(widget.snapshot.id.toString());
