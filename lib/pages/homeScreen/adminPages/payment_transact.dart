...
class _payment_transactState extends State<payment_transact> {
  ...
  init() async {
    ...
    for (DocumentSnapshot snapshot in querySnapshot.docs) {
      try {
        print("thara bhai jog");
        print((snapshot.data() as dynamic)["request"]);
        List requests = (snapshot.data() as dynamic)["request"] ?? [];
        for (var req in requests) {
          // Map<String, dynamic> userData = snapshot.data[];
          // print(req);
          var r = await req;
          var requestAmount = req["requestAmount"] ?? "0";
          if (int.parse(requestAmount) > 0 && req["status"] == "Processing") {
            r["registered_number"] = snapshot.id;
            usersList.add(r);
          }
        }
      } catch (e) {
            ...
            FlatButton(
                onPressed: () async {
                  ...
                  DocumentSnapshot res = await FirebaseFirestore.instance
                      .collection("Users")
                      .doc(snapshot["registered_number"])
                      .get();
                  print("Users/${snapshot["registered_number"]}");
                  print(res);
                  List requests = res.get("request"); //data["request"];
                  List new_requets = [];
                  ...
                  await FirebaseFirestore.instance
                      .collection("Users")
                      .doc(snapshot["registered_number"])
                      .set({"request": new_requets}, SetOptions(merge: true));
  ...
  Future<void> payService(var snapshot) async {
    print(DateTime.now().toString().split(" ")[0]); //await is removed from here
