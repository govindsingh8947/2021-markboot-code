...
class _TaskUserListPageState extends State<TaskUserListPage>
  ...
  late String _localPath; //late is added
  ...
  init() async {
    ...
    setState(() {
      try {
        status = (snap.data() as dynamic)["Status"];
      } catch (e) {
        print("!21212");
        status = true;
      }
          ...
          actions: [
            Switch(
                ...
                onChanged: (x) async {
                  ...
                  var dod = await FirebaseFirestore.instance
                      .collection("Posts")
                      .doc("Gigs")
                      .collection("Gigs")
                      .doc(widget.docId)
                  ...
                  if (dod.exists) {
                    FirebaseFirestore.instance
                        ...
                        .collection("Gigs")
                        .doc(widget.docId)
                        .set({"Status": x}, SetOptions(merge: true));
                  } else {
                    Fluttertoast.showToast(msg: "No Gig exsts");
                  }
                })
          ],
        ),
                  ...
                  if (task["userId"] == userId) {
                    ...
                    for (DocumentSnapshot docx in c) {
                      ...
                      if (t[t.indexOf(task)]["phoneNo"].toString() ==
                        ...
                        //print(docx.data);
                        print("in here ");
                        Map<String, dynamic> userData = docx.data() as Map<
                            String,
                            dynamic>; // as Map<String, dynamic>; is added
                  ...
                  if (task["userId"] == userId) {
                    ...
                    for (DocumentSnapshot docx in c) {
                      if (t[t.indexOf(task)]["phoneNo"].toString() ==
                          docx.id.toString()) {
                        print(docx.data);
                        Map<String, dynamic> userData = docx.data() as Map<String,
                            dynamic>; //as Map<String, dynamic>; is added
                                      ...
                                      if (reason != "") {
                                        ...
                                        for (var task in widget.taskUserList) {
                                          if (task["userId"] == userId) {
                                            ...
                                            var t = widget.taskUserList;
                                            setState(() {
                                              t[t.indexOf(task)]["status"] =
                                                  "rejected";
                                              phone = task["phoneNo"];
                                              t[t.indexOf(task)]["reason"] =
                                                  reason;
                                            });
                                            print(t);
...
//
                                        ...
                                        DocumentSnapshot snapshot =
                                            await FirebaseFirestore.instance
                                                ...
                                                .get();
                                        try {
                                          Map appliedInternshipList = (snapshot
                                                      .data()
                                                  as dynamic)["pendingTask"] ??
                                              Map();
                                          debugPrint(
                                              "LLn $appliedInternshipList ${widget.docId}");
                                          // appliedInternshipList[widget.docId] = true;
                                          appliedInternshipList
                                              .remove(widget.docId);
                                          snapshot.get("pendingList")[
                                                  "pendingList"] =
                                              appliedInternshipList; //this line is replaced and is errorable
                                          //snapshot.data["pendingTask"] = appliedInternshipList;
                                          debugPrint(
                                              "LLIUUUSSSTT ${snapshot.data}");
