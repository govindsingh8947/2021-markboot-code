...
class OngoingDetailScreen extends StatefulWidget {
  ...
  OngoingDetailScreen({
    required this.phone, //@required is replaced in all fields
    required this.name,
    required this.email,
    required this.task,
...
class _OngoingDetailScreenState extends State<OngoingDetailScreen> {
  ...
  verify() {
    ...
    if (value == null) {
      ...
      print(docID);
      var ref = _firestore.collection("Posts/Gigs/Campaign Tasks").doc(docID);
      ref.get().then((snapshot) {
        List<dynamic> list = List.from((snapshot.data()
            as dynamic)["submittedBy"]); //next line is replaced
        ...
        // list[i]["UserStatus"] = value;
        ref.update({'submittedBy': list}).catchError((e) {
          //updaeData is replaced
          print(e);
        });
      });
      //_onRefresh();
      print("ALULULUI");
      print(maps[0]);
      print(maps[0]["phoneNo"]);
      print(maps[0]["reward"]);
      _firestore.collection("Users").doc(maps[0]["phoneNo"]).set(
          {"approvedAmount": FieldValue.increment(int.parse(maps[0]["reward"]))},
      ...
      ref.get().then((snapshot) {
        List<dynamic> list =
            List.from((snapshot.data() as dynamic)["submittedBy"]);
        //List<dynamic> list = List.from(snapshot.data['submittedBy']);
        //if you need to update all positions of the array do a foreach instead of the next line
        int i = 0;
        for (i = 0; i < list.length; i++) {
  ...
  init() async {
    ...
    for (DocumentSnapshot snap in snaps) {
      List applys = (snap as dynamic)["submittedBy"];
